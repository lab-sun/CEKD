...
from TSmodel import Student_model,Teacher_model

#############################################################################################
parser = argparse.ArgumentParser(description='Test with pytorch')
#############################################################################################
parser.add_argument('--model_name', '-m', type=str, default='Teacher_model')
parser.add_argument('--weight_name', '-w', type=str, default='Teacher_model') # RTFNet_152, RTFNet_50, please change the number of layers in the network file
...
if __name__ == '__main__':
    ...
    #model = DeepLabV3Plus(encoder_name="resnet152",in_channels=3,classes=args.n_class)
    #model = UnetPlusPlus(encoder_name="resnet152",in_channels=4,classes=args.n_class)
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            if args.model_name=='Teacher_model':
                Tfeatures,logits,Thint = model(images)  # for teacher network
            else:
                Sfeatures,logits,Shint = model(thermal)  # for student network
            torch.cuda.synchronize()
            end_time = time.time()
    ...
    print("* F1 per class: \n    unlabeled: %.6f, car: %.6f, person: %.6f, bike: %.6f, curve: %.6f, car_stop: %.6f, guardrail: %.6f, color_cone: %.6f, bump: %.6f" \
          %(F1_per_class[0], F1_per_class[1], F1_per_class[2], F1_per_class[3], F1_per_class[4], F1_per_class[5], F1_per_class[6], F1_per_class[7], F1_per_class[8])) 




    print("\n* average values (np.mean(x)): \n iou: %.6f, recall: %.6f, pre: %.6f, F1: %.6f" \
          %(iou_per_class.mean(),recall_per_class.mean(), precision_per_class.mean(),F1_per_class.mean()))
    print("* average values (np.mean(np.nan_to_num(x))): \n iou: %.6f, recall: %.6f, pre: %.6f, F1: %.6f" \
          %(np.mean(np.nan_to_num(iou_per_class)), np.mean(np.nan_to_num(recall_per_class)), np.mean(np.nan_to_num(precision_per_class)), np.mean(np.nan_to_num(F1_per_class))))
