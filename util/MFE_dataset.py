...
class MFE_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        edges = self.read_image(name,'edge_4x')
        ...
        edges = np.asarray(PIL.Image.fromarray(edges).resize((self.input_w//4, self.input_h//4), resample=PIL.Image.NEAREST))
        edges = edges.astype('int64')
        
        return torch.tensor(image), torch.tensor(label), torch.tensor(edges), name
