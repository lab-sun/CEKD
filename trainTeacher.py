...
#parser.add_argument('--baseline_weight', '-bw', type=str, default='base50.pth')   #  Pre-weight for trianing the teacher network.
    ...
    for it, (images, rgb_labels, edge_labels, names) in enumerate(train_loader):
        ...
        if accIter['train'] % 20 == 0:
            if view_figure:
                ...
                #print(edge_result.size())
                #edge_predicted_tensor = edge_result* scale # mini_batch*args.n_class*480*640 -> mini_batch*480*640 -> mini_batch*1*480*640
...
def testing(epo, model, test_loader):
    ...
    if epo==0:
        with open(testing_results_file, 'w') as f:
            ...
            f.write("# epoch: unlabeled, car, person, bike, curve, car_stop, guardrail, color_cone, bump, average(nan_to_num). (Acc %, IoU %, Pre %, F1 % )\n")
