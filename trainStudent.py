...
def train(epo, Tmodel, Smodel, train_loader, optimizer):
    ...
    for it, (images, rgb_labels, edge_labels, names) in enumerate(train_loader):
        ...
        loss_sim = sim_dis_compute(Sfeatures,Tfeatures)
        loss_kdce = kd_ce_loss(Slogits,Tlogits,temperature=args.temperature)
        loss_edge = F.mse_loss(Shint,Thint)
        loss_seg = F.cross_entropy(Slogits,Tlabel)
        loss = loss_sim+loss_kdce+loss_edge+loss_seg
        loss.backward()
        optimizer.step()
        lr_this_epo=0
        ...
        print('Train: %s, epo %s/%s, iter %s/%s, lr %.8f, %.2f img/sec, loss %.4f, lossf %.4f, lossseg %.4f , lossh %.4f, lossl %.4f, time %s' \
            % (args.Smodel_name, epo, args.epoch_max, it+1, len(train_loader), lr_this_epo, len(names)/(time.time()-start_t), float(loss), float(loss_sim),float(loss_kdce), float(loss_edge),float(loss_seg),
        ...
        if accIter['train'] % 1 == 0:
            ...
            writer.add_scalar('Train/lossf', loss_sim, accIter['train'])
            writer.add_scalar('Train/lossseg', loss_kdce, accIter['train'])
            writer.add_scalar('Train/lossh', loss_edge, accIter['train'])
